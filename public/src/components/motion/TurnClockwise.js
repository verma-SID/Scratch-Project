...
const TurnClockWise = ({ character, characterAngle, comp_id }) => {
  ...
  return (
    ...
    style={t1} className="flex justify-center items-center">
      <div 
        className={`text-center grid grid-cols-1 text-sm gap-1 items-center justify-center z-[-100px] rounded-lg text-white cursor-pointer mx-auto`}>
        <div className="flex flex-row w-40 justify-between">
          <div className="text-white">Rotate By:</div>
          <input
            className="w-16 text-center text-black rounded-lg"
            type="number"
            value={angle}
        ...
        <div
          id={comp_id}
          className={`flex w-40 flex-row rounded-lg text-white text-sm cursor-pointer`}
          onClick={() => handleClick()}
        >
          <div className="flex flex-row w-40 justify-between">
            Turn
            <RedoIcon /> {angle} degrees
          </div>
        </div>
      </div>
    </div>
